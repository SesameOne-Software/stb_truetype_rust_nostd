...
pub unsafe fn stbtt__fill_active_edges_new(mut scanline: *mut f32, mut scanline_fill: *mut f32, mut len: i32, mut e: *mut stbtt__active_edge, mut y_top: f32) {
        ...
        if (*e).fdx == ((0) as f32) {
            ...
            if x0 < ((len) as f32) {
                if x0 >= ((0) as f32) {
                    ...
                    stbtt__handle_clipped_edge((scanline_fill).offset(-((1) as isize)), ((x0) as i32) + 1, e, x0, y_top, x0, y_bottom);
                } else {
                    stbtt__handle_clipped_edge((scanline_fill).offset(-((1) as isize)), 0, e, x0, y_top, x0, y_bottom);
                }
            }
        } else {
            let mut x0: f32 = (*e).fx;
            ...
            if x_top >= ((0) as f32) && x_bottom >= ((0) as f32) && x_top < ((len) as f32) && x_bottom < ((len) as f32) {
                if ((x_top) as i32) == ((x_bottom) as i32) {
                    let mut height: f32 = 0;
                    let mut x: i32 = ((x_top) as i32);
                    height = (((sy1 - sy0) * (*e).direction) as f32);
                    *scanline.offset((x) as isize) += (stbtt__position_trapezoid_area(height, x_top, ((x) as f32) + 1.0f32, x_bottom, ((x) as f32) + 1.0f32));
                    *scanline_fill.offset((x) as isize) += ((height) as f32);
                    ...
                    if y_crossing > y_bottom { y_crossing = ((y_bottom) as f32); }
                    sign = (((*e).direction) as f32);
                    area = ((sign * (y_crossing - sy0)) as f32);
                    *scanline.offset((x1) as isize) += (stbtt__sized_triangle_area(area, ((x1 + 1) as f32) - x_top));
                    if y_final > y_bottom {
                    ...
                    *scanline.offset((x2) as isize) += (area + sign * stbtt__position_trapezoid_area(sy1 - y_final, ((x2) as f32), ((x2) as f32) + 1.0f32, x_bottom, ((x2) as f32) + 1.0f32));
...
pub unsafe fn stbtt__handle_clipped_edge(mut scanline: *mut f32, mut x: i32, mut e: *mut stbtt__active_edge, mut x0: f32, mut y0: f32, mut x1: f32, mut y1: f32) {
    if y0 == y1 { return; }
    ...
    if x0 <= ((x) as f32) && x1 <= ((x) as f32) { *scanline.offset((x) as isize) += (((*e).direction * (y1 - y0)) as f32); } else {
        if x0 >= ((x + 1) as f32) && x1 >= ((x + 1) as f32) {} else {
            *scanline.offset((x) as isize) += ((*e).direction * (y1 - y0) * (((1) as f32) - ((x0 - ((x) as f32)) + (x1 - ((x) as f32))) / ((2) as f32)));
        }
    }
}

pub unsafe fn stbtt__rasterize(mut result: *mut stbtt__bitmap, mut pts: *mut stbtt__point, mut wcount: *mut i32, mut windings: i32, mut scale_x: f32, mut scale_y: f32, mut shift_x: f32, mut shift_y: f32, mut off_x: i32, mut off_y: i32, mut invert: i32, mut userdata: *mut u8) {
    ...
    e = ((c_runtime::malloc(std::mem::size_of::<stbtt__edge>() as u64 * ((n + 1) as u64))) as *mut stbtt__edge);
    if e == std::ptr::null_mut() { return; }
    n = ((0) as i32);
    m = ((0) as i32);
    i = ((0) as i32);
    ;
    while (i < windings) {
        ...
        while (k < *wcount.offset((i) as isize)) {
            let mut a: i32 = k;
            let mut b: i32 = j;
            if *p.offset((j) as isize).y == *p.offset((k) as isize).y { continue; }
            *e.offset((n) as isize).invert = ((0) as i32);
            if (if (invert) != 0 { *p.offset((j) as isize).y > *p.offset((k) as isize).y } else { *p.offset((j) as isize).y < *p.offset((k) as isize).y }) != 0 {
                *e.offset((n) as isize).invert = ((1) as i32);
                a = ((j) as i32);
                b = ((k) as i32);
            }
            *e.offset((n) as isize).x0 = ((*p.offset((a) as isize).x * scale_x + shift_x) as f32);
            *e.offset((n) as isize).y0 = ((*p.offset((a) as isize).y * y_scale_inv + shift_y) * ((vsubsample) as f32));
            *e.offset((n) as isize).x1 = ((*p.offset((b) as isize).x * scale_x + shift_x) as f32);
            *e.offset((n) as isize).y1 = ((*p.offset((b) as isize).y * y_scale_inv + shift_y) * ((vsubsample) as f32));
...
pub unsafe fn stbtt__rasterize_sorted_edges(mut result: *mut stbtt__bitmap, mut e: *mut stbtt__edge, mut n: i32, mut vsubsample: i32, mut off_x: i32, mut off_y: i32, mut userdata: *mut u8) {
    let mut hh: stbtt__hheap = [std::ptr::null_mut(), std::ptr::null_mut(), 0];
    ...
    if (*result).w > 64 { scanline = ((c_runtime::malloc((((*result).w * 2 + 1) as u64) * std::mem::size_of::<f32>() as u64)) as *mut f32); } else { scanline = scanline_data.as_mut_ptr(); }
    ...
    *e.offset((n) as isize).y0 = (((off_y + (*result).h) as f32) + ((1) as f32));
    while (j < (*result).h) {
        ...
        let mut step: * *mut stbtt__active_edge = &mut active;
        c_runtime::memset(((scanline) as *mut u8), 0, (((*result).w) as u64) * std::mem::size_of::<f32>() as u64);
        c_runtime::memset(((scanline2) as *mut u8), 0, (((*result).w + 1) as u64) * std::mem::size_of::<f32>() as u64);
        ...
        while ((*e).y0 <= scan_y_bottom) {
            if (*e).y0 != (*e).y1 {
                let mut z: *mut stbtt__active_edge = stbtt__new_active(((&mut hh) as *mut stbtt__hheap), e, off_x, scan_y_top, userdata);
        ...
        if (active) != std::ptr::null_mut() { stbtt__fill_active_edges_new(scanline, (scanline2).offset((1) as isize), (*result).w, active, scan_y_top); }
        {
            ...
            while (i < (*result).w) {
                ...
                k = (((c_runtime::fabs(((k) as f64))) as f32) * ((255) as f32) + 0.5f32);
                m = ((k) as i32);
                if m > 255 { m = ((255) as i32); }
    ...
    if scanline != scanline_data.as_mut_ptr() { c_runtime::free(((scanline) as *mut u8)); }
}
...
pub unsafe fn stbtt__sort_edges_quicksort(mut p: *mut stbtt__edge, mut n: i32) {
    while (n > 12) {
        ...
        let mut j: i32 = 0;
        m = ((n >> 1) as i32);
        c01 = ((if (&mut *p.offset((0) as isize)).y0 < (&mut *p.offset((m) as isize)).y0 { 1 } else { 0 }) as i32);
        c12 = ((if (&mut *p.offset((m) as isize)).y0 < (&mut *p.offset((n - 1) as isize)).y0 { 1 } else { 0 }) as i32);
        if c01 != c12 {
            let mut z: i32 = 0;
            c = ((if (&mut *p.offset((0) as isize)).y0 < (&mut *p.offset((n - 1) as isize)).y0 { 1 } else { 0 }) as i32);
        ...
        *p.offset((m) as isize) = ((t) as stbtt__edge);
        ...
        while (true) {
            ;
            while (true) {
                if !((&mut *p.offset((i) as isize)).y0 < (&mut *p.offset((0) as isize)).y0) { break; }
                c_runtime::preInc(&mut i);
            };
            while (true) {
                if !((&mut *p.offset((0) as isize)).y0 < (&mut *p.offset((j) as isize)).y0) { break; }
                c_runtime::preDec(&mut j);
            }
            if i >= j { break; }
...
pub unsafe fn stbtt_Rasterize(mut result: *mut stbtt__bitmap, mut flatness_in_pixels: f32, mut vertices: *mut stbtt_vertex, mut num_verts: i32, mut scale_x: f32, mut scale_y: f32, mut shift_x: f32, mut shift_y: f32, mut x_off: i32, mut y_off: i32, mut invert: i32, mut userdata: *mut u8) {
    ...
    let mut windings: *mut stbtt__point = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, ((&mut winding_lengths) as * *mut i32), ((&mut winding_count) as *mut i32), userdata);
