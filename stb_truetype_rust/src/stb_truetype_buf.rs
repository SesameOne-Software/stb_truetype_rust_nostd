...
pub unsafe fn stbtt__buf_get8(mut b: *mut stbtt__buf) -> u8 {
    if (*b).cursor >= (*b).size { return ((0) as u8); }
    return ((*(*b).data.offset((c_runtime::postInc(&mut (*b).cursor)) as isize)) as u8);
}
...
pub unsafe fn stbtt__buf_range(mut b: *mut stbtt__buf, mut o: i32, mut s: i32) -> stbtt__buf {
    ...
    if o < 0 || s < 0 || o > (*b).size || s > (*b).size - o { return ((r) as stbtt__buf); }
...
pub unsafe fn stbtt__cff_int(mut b: *mut stbtt__buf) -> u32 {
    ...
    if b0 >= 32 && b0 <= 246 { return ((b0 - 139) as u32); } else { if b0 >= 247 && b0 <= 250 { return (((b0 - 247) * 256 + ((stbtt__buf_get8(b)) as i32) + 108) as u32); } else { if b0 >= 251 && b0 <= 254 { return ((-(b0 - 251) * 256 - ((stbtt__buf_get8(b)) as i32) - 108) as u32); } else { if b0 == 28 { return ((stbtt__buf_get((b), 2)) as u32); } else { if b0 == 29 { return ((stbtt__buf_get((b), 4)) as u32); } } } } }
...
pub unsafe fn stbtt__dict_get(mut b: *mut stbtt__buf, mut key: i32) -> stbtt__buf {
    ...
    while ((*b).cursor < (*b).size) {
        ...
        let mut end: i32 = 0;
        let mut op: i32 = 0;
        while (((stbtt__buf_peek8(b)) as i32) >= 28) { stbtt__cff_skip_operand(b); }
        end = (((*b).cursor) as i32);
        op = ((stbtt__buf_get8(b)) as i32);
        if op == 12 { op = (((stbtt__buf_get8(b)) as i32) | 0x100); }
        if op == key { return ((stbtt__buf_range(b, start, end - start)) as stbtt__buf); }
    }
    return ((stbtt__buf_range(b, 0, 0)) as stbtt__buf);
}

pub unsafe fn stbtt__dict_get_ints(mut b: *mut stbtt__buf, mut key: i32, mut outcount: i32, mut out: *mut u32) {
    let mut i: i32 = 0;
