...
pub unsafe fn stbtt__CompareUTF8toUTF16_bigendian_prefix(
    ...
    let mut i: i32 = 0;
    while ((len2) != 0) {
        let mut ch: u16 = ((((*s2.offset((0) as isize)) as i32) * 256
        ...
        if ((ch) as i32) < 0x80 {
            ...
            if ((ch) as i32) < 0x800 {
                ...
                if ((ch) as i32) >= 0xd800 && ((ch) as i32) < 0xdc00 {
                    let mut c: u32 = 0;
                    let mut ch2: u16 = ((((*s2.offset((2) as isize)) as i32) * 256
...
pub unsafe fn stbtt__compute_crossings_x(
    ...
    while (i < nverts) {
        ...
        if (((*verts.offset((i) as isize))._type_) as i32) == STBTT_vcurve {
            ...
            let mut ax: i32 = (if (x0) < (if (x1) < (x2) { (x1) } else { (x2) }) {
                (x0)
            } else {
                (if (x1) < (x2) { (x1) } else { (x2) })
            });
            let mut ay: i32 = (if (y0) < (if (y1) < (y2) { (y1) } else { (y2) }) {
                (y0)
            } else {
                (if (y1) < (y2) { (y1) } else { (y2) })
            });
            let mut by: i32 = (if (y0) < (if (y1) < (y2) { (y2) } else { (y1) }) {
                (if (y1) < (y2) { (y2) } else { (y1) })
            } else {
                (y0)
            });
            if y > ((ay) as f32) && y < ((by) as f32) && x > ((ax) as f32) {
                ...
                let mut hits: [[f32; 2]; 2] = [[0.0f32; 2]; 2];
                    ...
                    )) != 0
                {
                    ...
                    if y > ((if (y0) < (y1) { (y0) } else { (y1) }) as f32)
                        ...
                        && x > ((if (x0) < (x1) { (x0) } else { (x1) }) as f32)
                    {
                        let mut x_inter: f32 = (y - ((y0) as f32)) / ((y1 - y0) as f32)
                            * ((x1 - x0) as f32)
                            + ((x0) as f32);
                        if x_inter < x {
                            winding += ((if (y0 < y1) { 1 } else { -1 }) as i32);
...
pub unsafe fn stbtt__find_table(mut data: *const u8, mut fontstart: u32, mut tag: &str) -> u32 {
    ...
    i = ((0) as i32);
    while (i < num_tables) {
        let mut loc: u32 = tabledir + ((16 * i) as u32);
...
pub unsafe fn stbtt__get_subr(mut idx: stbtt__buf, mut n: i32) -> stbtt__buf {
    ...
    n += ((bias) as i32);
    if n < 0 || n >= count {
        return ((stbtt__new_buf(std::ptr::null_mut(), ((0) as u64))) as stbtt__buf);
    }
...
pub unsafe fn stbtt__matches(
    ...
    let mut nlen: i32 = ((c_runtime::strlen(name)) as i32);
    let mut nm: u32 = 0;
    let mut hd: u32 = 0;
...
pub unsafe fn stbtt__ray_intersect_bezier(
    ...
    if ((a) as f64) != 0.032 {
        ...
        if ((discr) as f64) > 0.032 {
            ...
            let mut d: f32 = c_runtime::sqrt(discr);
            s0 = (((b + d) * rcpna) as f32);
            s1 = (((b - d) * rcpna) as f32);
...
pub unsafe fn stbtt__solve_cubic(mut a: f32, mut b: f32, mut c: f32, mut r: *mut f32) -> i32 {
    ...
    if d >= ((0) as f32) {
        let mut z: f32 = c_runtime::sqrt(d);
        ...
        *r.offset((0) as isize) = ((s + u + v) as f32);
        ...
        let mut u: f32 = ((c_runtime::sqrt(((-p / ((3) as f32)) as f32))) as f32);
        let mut v: f32 = ((c_runtime::acos(
            -c_runtime::sqrt(((((-27) as f32) / p3) as f32)) * ((q) as f32) / ((2) as f32),
        )) as f32)
            / ((3) as f32);
        let mut m: f32 = ((c_runtime::cos(((v) as f32))) as f32);
        let mut n: f32 =
...
pub unsafe fn stbtt__tesselate_cubic(
    ...
    let mut longlen: f32 = ((c_runtime::sqrt(((dx0 * dx0 + dy0 * dy0) as f32))
        + c_runtime::sqrt(((dx1 * dx1 + dy1 * dy1) as f32))
        + c_runtime::sqrt(((dx2 * dx2 + dy2 * dy2) as f32))) as f32);
    let mut shortlen: f32 = ((c_runtime::sqrt(((dx * dx + dy * dy) as f32))) as f32);
...
pub unsafe fn stbtt_BakeFontBitmap(
    ...
    mut first_char: i32,
    mut num_chars: i32,
    mut chardata: *mut stbtt_bakedchar,
) -> i32 {
...
pub unsafe fn stbtt_FlattenCurves(
    ...
    *contour_lengths =
        ((c_runtime::malloc(std::mem::size_of::<i32>() as u64 * ((n) as u64))) as *mut i32);
        ...
        while (pass < 2) {
            ...
            if pass == 1 {
                points = ((c_runtime::malloc(
                    ((num_points) as u64) * std::mem::size_of::<stbtt__point>() as u64,
                )) as *mut stbtt__point);
                if points == std::ptr::null_mut() {
                    break 'outer;
                }
...
pub unsafe fn stbtt_GetBakedQuad(
    ...
    mut xpos: *mut f32,
    mut ypos: *mut f32,
    mut q: *mut stbtt_aligned_quad,
    mut opengl_fillrule: i32,
) {
    let mut d3d_bias: f32 = if (opengl_fillrule) != 0 {
        ((0) as f32)
    } else {
        -0.5f32
    };
    ...
    let mut round_x: i32 = ((c_runtime::floor((((*xpos + (*b).xoff) + 0.5f32) as f32))) as i32);
    let mut round_y: i32 = ((c_runtime::floor((((*ypos + (*b).yoff) + 0.5f32) as f32))) as i32);
...
pub unsafe fn stbtt_GetPackedQuad(
    mut chardata: *mut stbtt_packedchar,
    mut pw: i32,
    mut ph: i32,
    mut char_index: i32,
    mut xpos: *mut f32,
