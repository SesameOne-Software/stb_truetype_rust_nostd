...
pub unsafe fn stbtt_PackBegin(
    ...
    let mut context: *mut stbrp_context =
        ((c_runtime::malloc(std::mem::size_of::<stbrp_context>() as u64)) as *mut stbrp_context);
    let mut num_nodes: i32 = pw - padding;
    let mut nodes: *mut stbrp_node =
        ((c_runtime::malloc(std::mem::size_of::<stbrp_node>() as u64 * ((num_nodes) as u64)))
            as *mut stbrp_node);
...
pub unsafe fn stbtt_PackFontRanges(
    ...
    rects = ((c_runtime::malloc(std::mem::size_of::<stbrp_rect>() as u64 * ((n) as u64)))
        as *mut stbrp_rect);
    if rects == std::ptr::null_mut() {
        return ((0) as i32);
    }
